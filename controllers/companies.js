"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.getCompanyInfo = exports.updateCompanyInfo = exports.setInitialCompanyInfo = void 0;
var models_1 = require("../models");
var helpers_1 = require("../helpers");
var setInitialCompanyInfo = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, user, existCompanyInfo, company, e_1;
    var _a;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                id = (_a = req.user) === null || _a === void 0 ? void 0 : _a._id;
                return [4 /*yield*/, models_1.User.findById(id)];
            case 1:
                user = _b.sent();
                return [4 /*yield*/, models_1.Company.find({
                        user: id
                    })];
            case 2:
                existCompanyInfo = _b.sent();
                console.log(existCompanyInfo);
                if (existCompanyInfo.length) {
                    return [2 /*return*/, res.status(201).json({
                            msg: 'mal uso de peticion'
                        })];
                }
                if (!user) {
                    return [2 /*return*/, res.status(200).json({
                            msg: 'Al perecer tu cuenta no esta activa'
                        })];
                }
                company = new models_1.Company({
                    user: id,
                    name: req.body.name,
                    description: req.body.description
                });
                _b.label = 3;
            case 3:
                _b.trys.push([3, 5, , 6]);
                return [4 /*yield*/, company.save()];
            case 4:
                _b.sent();
                return [2 /*return*/, res.status(201).json({
                        msg: 'datos guardados correctamente',
                        company: company
                    })];
            case 5:
                e_1 = _b.sent();
                (0, helpers_1.errorResponse)(e_1, res);
                return [3 /*break*/, 6];
            case 6: return [2 /*return*/];
        }
    });
}); };
exports.setInitialCompanyInfo = setInitialCompanyInfo;
var updateCompanyInfo = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, company;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = req.params.id;
                return [4 /*yield*/, models_1.Company.findByIdAndUpdate(id, __assign({}, req.body))];
            case 1:
                company = _a.sent();
                if (!company) {
                    return [2 /*return*/, res.status(200).json({
                            msg: 'Al perecer tu cuenta de companias no esta activa'
                        })];
                }
                return [2 /*return*/, res.status(200).json({
                        msg: 'datos actualizados correctamente'
                    })];
        }
    });
}); };
exports.updateCompanyInfo = updateCompanyInfo;
var getCompanyInfo = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, company;
    var _a;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                id = (_a = req.user) === null || _a === void 0 ? void 0 : _a._id;
                return [4 /*yield*/, models_1.Company.findOne({
                        user: id
                    })];
            case 1:
                company = _b.sent();
                if (!company) {
                    return [2 /*return*/, res.status(200).json({
                            msg: 'Al perecer tu cuenta de companias no esta activa'
                        })];
                }
                return [2 /*return*/, res.status(200).json({
                        company: company
                    })];
        }
    });
}); };
exports.getCompanyInfo = getCompanyInfo;
